  ...
  while page_number < 40
    ...
    while i < 20
      if Movie.exists?(:synopsis => movies.results[i].overview)
        i += 1
      elsif (movies.results[i].original_language == "en") && (movies.results[i].adult == false) && (movies.results[i].poster_path != nil) && (movies.results[i].release_date) && (movies.results[i].original_title != nil)     
